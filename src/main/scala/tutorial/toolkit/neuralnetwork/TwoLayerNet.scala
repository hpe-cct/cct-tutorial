...
object TwoLayerNet extends CogDebuggerApp(new ComputeGraph {
  ...
  val fc1 = FullyConnectedLayer(data, 32, lr)
  val r1 = ReLU(fc1)
  val fc2 = FullyConnectedLayer(r1, 10, lr)

  val loss = CrossEntropySoftmax(fc2, label)
  val normLoss = loss / batchSize
  val correct = CorrectCount(fc2.forward, label.forward, batchSize, 0.01f) / batchSize
  ...
  val valFc1 = FullyConnected(valData, fc1.weights)
  val valR1 = ReLU(valFc1)
  val valFc2 = FullyConnected(valR1, fc2.weights)

  val valCorrect = CorrectCount(valFc2.forward, valLabel.forward, valBatchSize, 0.01f) / valBatchSize
  val valAvgCorrect = NormalizedLowPass(valCorrect, 0.001f)

  normLoss.activateSGD()

  probe(fc1.weights.forward)
  fc1.weights.backward.foreach(probe(_, "fc1 update"))

  probe(fc2.weights.forward)
  fc2.weights.backward.foreach(probe(_, "fc2 update"))
