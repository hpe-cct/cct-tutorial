...
object AlexNet extends CogDebuggerApp(new ComputeGraph (optimize = false) {
  //object AlexNet extends App {
  ...
  val r1 = ReLU(b1)
  val m1 = MaxPooling(r1, poolSize = 3, stride = 2)

  val c2 = ConvolutionLayer(m1, Shape(5, 5), 256, BorderValid, lr, stride = 1)
  val b2 = BiasLayer(c2, lr)
  val r2 = ReLU(b2)
  val m2 = MaxPooling(r2, poolSize = 3, stride = 2)

  val c3 = ConvolutionLayer(m2, Shape(3, 3), 384, BorderValid, lr, stride = 1)
  val b3 = BiasLayer(c3, lr)
  val r3 = ReLU(b3)

  val c4 = ConvolutionLayer(r3, Shape(3, 3), 384, BorderValid, lr, stride = 1)
  val b4 = BiasLayer(c4, lr)
  val r4 = ReLU(b4)

  val c5 = ConvolutionLayer(r4, Shape(3, 3), 256, BorderValid, lr, stride = 1)
  val b5 = BiasLayer(c5, lr)
  val r5 = ReLU(b5)
  val m5 = MaxPooling(r5, poolSize = 3, stride = 2)


  val fc6 = FullyConnectedLayer(m5, 4096, lr)
